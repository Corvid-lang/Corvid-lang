...
- [ ] 21-inv-H-4-follow       Upgrade `ReceiptNarrative` to `Grounded<ReceiptNarrative>` now that 22-F ships the provenance-handle path. Rust-side `ReceiptNarrative` becomes `Grounded<_>`-wrapped via 22-F-Level-2 host minting (requires 22-F follow-up `22-F-host-mint`) or a new Corvid primitive for minting from plain values. Pick whichever lands first.
- [ ] 21-inv-H-5-custom-policy  Promote the default regression policy from a Rust function to a user-replaceable `.cor` program. `--policy=<path>` flag loads + compiles the user's `apply_policy(receipt) -> Verdict` agent; default policy ships as `default_policy.cor` baked into the CLI. Governance-as-code for the gate itself.
- [ ] 21-inv-H-5-signed        DSSE-signed receipts via `--sign=<key-id>`; `corvid trace-diff --verify <url-or-path>` round-trips. `corvid receipt show <hash>` resolves against a local cache. Turns the receipt from informational text into a cryptographic audit artifact.
- [ ] 21-inv-H-5-in-toto       SLSA/Sigstore in-toto attestation renderer (`--format=in-toto`). Corvid receipts plug into supply-chain tooling as proof-of-governance alongside traditional SBOM dependency scanners.
...
- [ ] lang-cor-imports-basic-resolve-2c-2 **Owner preference: Dev B.** Real type resolution for successful qualified lookups. Design call (before code): (A) new `Type::ImportedStruct` variant — clean, ripples through ~15 pattern-match sites across ~10 crates; or (B) re-register imports as synthetic DefIds in current file's symbol table — less ripple, blurs file-boundary invariants. Needs `DeclExport` to carry struct fields; may need `ResolvedModule` to carry `Arc<File>`. Update `check_field`, `type_ref_to_type::Qualified` Found arm, `Type::display_name`.
- [ ] lang-cor-imports-basic-resolve-2c-3 **Owner preference: Dev B.** Driver integration: every production path that today calls `typecheck(file, resolved)` should call `typecheck_with_modules` when the file has Corvid imports. Relatively small (~100–150 lines across driver entry points) but gated on 2c-2 landing first.
- [ ] lang-cor-imports-basic         ROLL-UP — closes when 2c-2 + 2c-3 both land.
