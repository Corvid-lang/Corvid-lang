...
- [ ] 21-inv-H-4-follow      Upgrade `ReceiptNarrative` to `Grounded<ReceiptNarrative>` once 22-F ships. Blocks on the FFI-side `Grounded<T>` provenance-handle path — Rust cannot mint a grounded value today without either a new language primitive (scope creep) or boundary support for host-asserted grounding (22-F Level 2).
