...
- [ ] 25-D-registry-http-contract     Minimal stateless registry API contract, local conformance harness, cache headers, immutable artifact URL rules, and CI fixture proving a registry mirror can serve `index.toml` plus source artifacts without trusted server logic.
- [ ] 25-E-package-metadata-pages     Package metadata renderer exposing exports, effect profiles, approval boundaries, provenance guarantees, replay/determinism guarantees, signatures, and install snippets for CLI and web pages.
- [ ] 25-F-conflict-resolution        Multi-dependency semver conflict detection, lockfile graph validation, and diagnostics when two dependencies require incompatible package versions or effect policies.
