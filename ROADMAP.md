...
- [ ] lang-cor-imports-versioned ROLL-UP — versioned imports + package system: `import "corvid://@anthropic/safety-baseline/v2.3" as p`. Closes only when locked package resolution, registry semantic-version selection, and signed publish verification all land. Large scope; split below so none of the package-manager story is faked.
...
- [ ] lang-cor-imports-versioned-registry Registry semantic-version resolver: `corvid add @scope/name@2.3` queries a registry index, chooses a version by semver policy, writes `Corvid.lock`, records the semantic summary, and refuses packages whose exported effects violate project policy.
- [ ] lang-cor-imports-versioned-signed-publish Signed publish workflow: source package artifacts carry signatures over bytes + semantic summary + effect metadata; install/publish verifies author identity and produces actionable diagnostics on signature or summary drift.
