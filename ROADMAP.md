...
- [ ] **17b — principled RC optimization (Perceus).** *Region inference dropped from this slice based on Perceus paper analysis + MLton's published rejection of Tofte-Talpin regions; revisit only if 17b-1/2/3 measurements show remaining allocation pressure justifies the complexity.* Sub-slices:
  - [x] **17b-0** *(landed 2026-04-15)* — `corvid_retain_call_count` + `corvid_release_call_count` runtime instrumentation; baseline RC op counts recorded as exact-match assertions on 5 representative workloads.
  - [ ] **17b-1** — principled `dup`/`drop` insertion pass replacing ad-hoc codegen-time emission. Per-callee borrow inference (parameters that are only forwarded/read are passed borrowed, eliminating one retain/release pair per call). Split into two sub-commits:
    - [x] **17b-1a** *(landed 2026-04-15)* — `IrStmt::Dup` / `IrStmt::Drop` as first-class IR variants; `ParamBorrow` enum + `IrAgent.borrow_sig` field; codegen handles the new variants end-to-end. Pure scaffolding, behavior-preserving (baseline RC counts unchanged).
    - [ ] **17b-1b** — ownership analysis pass populating Dup/Drop + borrow_sig; replaces scattered `emit_retain`/`emit_release` calls in codegen with analysis-driven insertion. Ships the measurable RC-op reduction on the 17b-0 baselines.
  ...
  - [ ] **17b-3** — reuse analysis. Match `drop(x_size_N); alloc(size_M ≤ N)` pairs in a basic block; emit `if (refcount & MASK) == 1 { reuse_in_place } else { drop; alloc }`. Same-size-in-words rule per Perceus.
