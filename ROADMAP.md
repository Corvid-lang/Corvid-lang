...
- [ ] lang-cor-imports-basic   `import "./path" as alias` with qualified access (`alias.TypeName`, `alias.agent_name(...)`). Module graph + cycle detection. Relative-path resolution. Type checker resolves qualified names via module symbol tables. The load-bearing core of the `.cor` import story. Unblocks `21-inv-H-5-custom-policy` for the ambitious shape.
