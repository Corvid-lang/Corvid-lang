...
- [ ] **17b — principled RC optimization (Perceus).** *Region inference dropped from this slice based on Perceus paper analysis + MLton's published rejection of Tofte-Talpin regions; revisit only if 17b-1/2/3 measurements show remaining allocation pressure justifies the complexity.* Sub-slices:
  ...
  - [ ] **17b-1** — principled `dup`/`drop` insertion pass replacing ad-hoc codegen-time emission. Per-callee borrow inference (parameters that are only forwarded/read are passed borrowed, eliminating one retain/release pair per call). Pre-phase chat first.
