...
- [ ] lang-cor-imports-basic-resolve  Module loader + cycle detection + qualified-name resolution in the type checker. `alias.TypeName` / `alias.agent_name(...)` resolve to the imported module's symbols. Visibility becomes load-bearing (only `public` declarations are accessible via qualified access). The load-bearing core of the `.cor` import story. Unblocks `21-inv-H-5-custom-policy`.
- [ ] lang-cor-imports-basic         ROLL-UP — closes when `-parse` + `-resolve` both land.
