...
pub use build::{
    ...
    NativeBuildOutput, SigningRequest, TargetBuildOutput, WasmBuildOutput,
};
...
///
/// Runs the full frontend (lex / parse / resolve / typecheck / lower)
/// plus the effect-registry build needed by `emit_abi`. Caller-
/// supplied `source_path_for_descriptor` goes into the descriptor's
/// `source_path` provenance field (forward-slash normalised to keep
/// Windows + Unix outputs byte-stable); `generated_at` is the RFC3339
/// timestamp rendered into the descriptor.
///
/// Used by tools that need to reason about a program's AI-safety
/// surface (effects, approval contracts, provenance, dispatch) without
/// actually emitting a library — notably `corvid trace-diff`, which
/// computes PR-level behavior deltas by comparing two descriptors.
